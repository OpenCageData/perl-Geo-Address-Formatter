...
developing your own configuration you should use (and contribute to)
those in
https://github.com/lokku/address-formatting. It includes test cases.
...
The only option you can set currently is 'country_code' which should
...
sub format_address {
  ...
  my $cc = $self->_determine_country_code($components) || $rh_options->{country} || '';
